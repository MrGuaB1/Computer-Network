...
bool recvSegment(Message&,int&); // 接收数据段
...
int main()
{
	...
	while (true) {
		...
		else if(fileMessage.getCheck() && fileMessage.seqNum != next){
			Message ackMessage;
			ackMessage.setPort(SERVER_PORT, CLIENT_PORT);
			ackMessage.set_ACK(); //ACK置位表示响应
			ackMessage.ackNum = next-1; // 回复的响应号就是接收的序列号
			ackMessage.setCheck(); // 设置校验和
			sendto(serverSocket, (char*)&ackMessage, sizeof(ackMessage), 0, (sockaddr*)&clientAddr, addrLen);
			cout << "server收到【失序的报文段】 seq = " << ackMessage.seqNum << "并发送 ack =" << ackMessage.ackNum << " 的回复报文段" << endl;
		}
	}
	...
	for (int i = 0; i < segments; i++) {
		Message fileData;
		if (recvSegment(fileData,next) == true)
...
// 累积确认：只确认连续正确接收分组的最大序列号
bool recvSegment(Message& recvMessage,int& next) {
	while (true) {
		int bytes = recvfrom(serverSocket, (char*)&recvMessage, sizeof(recvMessage), 0, (sockaddr*)&clientAddr, &addrLen);
		if (bytes > 0) {
			// 成功收到消息，检查ACK和seq，然后向客户端回复一个ACK：
			// lab3-2更改，检查序列号是否是当前期望的序列号
			// 在服务端维护的全局序列号仅用于建立连接和释放连接
			if (recvMessage.getCheck() && (recvMessage.seqNum == next)) {
				next++; // 期待下一个报文段
				Message ackMessage;
				ackMessage.setPort(SERVER_PORT, CLIENT_PORT);
				ackMessage.set_ACK(); //ACK置位表示响应
			...
			// 如果不是期待的序列号，说明发生了失序现象
			else if (recvMessage.getCheck() && (recvMessage.seqNum != next)) {
				Message ackMessage;
				ackMessage.setPort(SERVER_PORT, CLIENT_PORT);
				ackMessage.set_ACK(); //ACK置位表示响应
				ackMessage.ackNum = next-1; 
				ackMessage.setCheck(); // 设置校验和
				sendto(serverSocket, (char*)&ackMessage, sizeof(ackMessage), 0, (sockaddr*)&clientAddr,addrLen);
				cout << "server收到【失序的报文段】 seq = " << recvMessage.seqNum << "并发送 ack =" << ackMessage.ackNum << " 的回复报文段" << endl;
			}
		}
		else if (bytes == 0)
			return false;
